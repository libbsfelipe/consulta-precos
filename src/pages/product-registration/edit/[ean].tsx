...
export default function Example() {
    ...
    useEffect(() => {
        try{
            ...
            axios.get('/api/products/getByEan/' + ean).then((response) => {
                ...
                setPhotoPath(response.data.productImagePath);
            }).catch((error) => {
                console.log(error);
            });
        }
    ...
    const saveProduct = (event: FormEvent) => {
        try {
            ...
            api.put('/api/products/put/' + ean, formData)
            .then((response) => {
                setShowLoader(false);
                setShowSucess(true);
    ...
    return (
        <div>
            ...
            <ModalSucess showLoader={showSucess} setShowLoader={setShowSucess} text="Sucesso ao salvar produto." />
            <ModalError  showLoader={showError} setShowLoader={setShowError} text="Erro ao salvar produto." />
                                ...
                                <div>
                                    ...
                                    <div className="mt-1 flex items-center">
                                        <div className="inline-block h-48 w-40 overflow-hidden bg-gray-100 flex items-center">
                                            <img src={(photo != null && photo != '') ? URL.createObjectURL(photo) : photoPath} width="100%" height="100%" alt={photo != null ? photo.name : 'no-image.png'} />
                                        </div>
                                        <div>
                                            <input
